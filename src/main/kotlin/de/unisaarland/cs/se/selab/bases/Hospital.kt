...
class Hospital(id: Int, staff: Int, location: Vertex, vehicles: MutableList<Vehicle>, var doctors: Int) : Base(
    ...
    override fun checkCombination(em: Emergency, vehicles: MutableList<Vehicle>): Boolean {
        ...
        for (vec in vehicles) {
            ...
            if (vec.vehicleType == VehicleType.EMERGENCY_DOCTOR_CAR) {
                numberOfEmDocCars++
            }
        }
        if (staffNeeded > this.staff || numberOfEmDocCars > this.doctors) validCombination = false
        if (resource.criminalAmount - fittingCriminals > MaxCriminalCapacity *
            (resource.countInstancesOf(VehicleType.POLICE_CAR) - numberOfPoliceCars)
        ) {
            return false
        }

        if (resource.waterAmount -
            fittingWater > maxWaterCapacity * (
